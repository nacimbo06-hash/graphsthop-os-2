...
- **Money core (atomic transactions):** every multi-write money flow runs inside a single-connection Rust `#[tauri::command]` (one `pool.begin()` transaction), because `db.transaction()` over the plugin's connection pool was never atomic (upstream issue #886) — and that helper has been **removed** from `database.ts` so the pattern can't be reused. Commands live in `apps/desktop/src-tauri/src/`: `checkout_sale` (checkout.rs), `receive_goods`, `create_purchase_order`, `record_credit_transaction`, `adjust_stock`, `lot_ops` (`update_lot_with_movement` / `bulk_update_lot_statuses`), `treasury_ops` (`transfer_to_safe` / `contribute_to_fund` / `record_expense_payment`), `close_session`. Stores/the treasury facade call them via `invoke(...)` under Tauri (with an in-memory/localStorage fallback in plain-browser dev). Reads stay on plugin-sql `select()`; single-row writes use `db.insert/update/execute`. Each command has Rust `#[cfg(test)]` coverage. Pool access is borrowed from the plugin via `db.rs` (`DbInstances`). See the plan in `~/.claude/plans/`.
