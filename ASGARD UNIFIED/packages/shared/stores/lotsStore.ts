...
interface LotsState {
    ...
    // Actions
    addLot: (lot: Omit<ProductLot, 'id' | 'createdAt' | 'status' | 'daysRemaining'>) => string;
    updateLot: (id: string, updates: Partial<ProductLot>) => void;
    reduceLotQuantity: (lotId: string, quantity: number, reason: string, reference?: string, createdBy?: string) => void;
    disposeLot: (lotId: string, reason: string, createdBy: string) => void;
